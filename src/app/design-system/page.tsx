  ...
  const renderContent = () => {
    ...
    if (activeCategory === 'foundations') {
      switch (activeComponent) {
        case 'colors':
          return (
            <Layout title="컬러 시스템" description="DogNote 디자인 시스템의 컬러 팔레트">
              <ColorPalette />
            </Layout>
          );
        case 'typography':
          return (
            <Layout title="타이포그래피" description="폰트 패밀리, 크기, 가중치 시스템">
              <TypographyExamples />
            </Layout>
          );
        case 'spacing':
        ...
        case 'shadows':
          return (
            <Layout title="그림자" description="깊이와 계층을 표현하는 그림자 시스템">
              <ShadowExamples />
            </Layout>
          );
        default:
          return (
            <Layout title={activeComponent} description={`${activeComponent} 시스템`}>
              <div className="space-y-6">
                <h3 className="text-xl font-semibold">{activeComponent} 시스템</h3>
                <p>이 섹션은 준비 중입니다.</p>
              </div>
            </Layout>
          );
    ...
    if (activeCategory === 'components') {
      switch (activeComponent) {
        case 'buttons':
          return (
            <Layout title="버튼" description="사용자 액션을 유도하는 인터랙티브 요소">
              <ButtonExamples />
            </Layout>
          );
        case 'inputs':
          return (
            <Layout title="입력 필드" description="사용자 데이터 수집을 위한 폼 요소">
              <InputExamples />
            </Layout>
          );
        default:
          return (
            <Layout title={activeComponent} description={`${activeComponent} 컴포넌트`}>
              <div className="space-y-6">
                <h3 className="text-xl font-semibold">{activeComponent} 컴포넌트</h3>
                <p>이 컴포넌트는 준비 중입니다.</p>
              </div>
            </Layout>
          );
    ...
    if (activeCategory === 'utilities') {
      switch (activeComponent) {
        case 'theme':
          return (
            <Layout title="테마 유틸리티" description="테마 시스템 및 유틸리티 함수">
              <ThemeUtilities />
        ...
        default:
          return (
            <Layout title={activeComponent} description={`${activeComponent} 유틸리티`}>
              <div className="space-y-6">
                <h3 className="text-xl font-semibold">{activeComponent} 유틸리티</h3>
                <p>이 유틸리티는 준비 중입니다.</p>
              </div>
            </Layout>
          );
    ...
    // 기본 콘텐츠
    return (
      <Layout title="디자인 시스템" description="DogNote 디자인 시스템에 오신 것을 환영합니다">
        <div className="space-y-6">
