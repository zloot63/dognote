...
describe('SpacingExamples', () => {
  ...
  it('스페이싱 값 섹션이 렌더링되어야 함', () => {
    ...
    const spacingValuesSection = screen.getByText('스페이싱 값').closest('section');
    expect(spacingValuesSection).toBeInTheDocument();
    
    const tableHeaders = within(spacingValuesSection!).getAllByRole('columnheader');
  ...
  it('스페이싱 사용 예시 섹션이 렌더링되어야 함', () => {
    ...
    const usageSection = screen.getByText('스페이싱 사용 예시').closest('section');
  ...
  it('복사 버튼 클릭 시 복사 함수가 호출되어야 함', () => {
    ...
    expect(screen.getByTestId('copy-button-p-4').querySelector('svg')).toBeInTheDocument();
  });
});
