...
export function useColorContrast() {
   ...
   * WCAG AA 준수 여부 확인
   */
  const isWCAGCompliant = useCallback((color1: string, color2: string, isLargeText = false): {
    isCompliant: boolean;
    ratio: number;
    level: 'AA' | 'AAA' | 'FAIL';
  } => {
...
export function useResponsiveAccessibility() {
  ...
  useEffect(() => {
    const checkScreenSize = () => {
      ...
      // 모바일에서 더 큰 터치 타겟 권장
      if (width < 768) {
        document.documentElement.style.setProperty('--touch-target-min-size', '48px');
      } else {
        document.documentElement.style.setProperty('--touch-target-min-size', '32px');
      }
    };
