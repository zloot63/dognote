...
const WalkTracker: React.FC<WalkTrackerProps> = ({
    ...
    if (walkState.isActive && walkState.startTime) {
      durationIntervalRef.current = setInterval(() => {
        ...
        const duration = Math.floor(
          (now.getTime() - walkState.startTime!.getTime()) / 1000
        );
  ...
  const handleStartWalk = async () => {
    ...
    try {
      ...
      const selectedDogIds = dogs && dogs.length > 0 ? [dogs[0].id] : [];
      ...
      console.warn('산책 시작:', { walkId, startTime, selectedDogIds });
    } catch (error) {
      console.error('산책 시작 실패:', error);
      setError(
        error instanceof Error ? error.message : '산책 시작에 실패했습니다.'
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleEndWalk = async () => {
    ...
    try {
      ...
      const finalDuration = Math.floor(
        (endTime.getTime() - walkState.startTime!.getTime()) / 1000
      );
      ...
      const completedWalk = {
        id: walkState.walkId,
        userId: '',
        dogIds: walkState.selectedDogIds,
        startTime: walkState.startTime!.toISOString(),
        endTime: endTime.toISOString(),
        duration: finalDuration,
        distance: finalDistance,
        route: walkState.positions.map(pos => ({ lat: pos.lat, lng: pos.lng })),
      ...
      console.warn('산책 종료:', completedWalk);
      ...
      setError(
        error instanceof Error ? error.message : '산책 종료에 실패했습니다.'
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleRequestPermission = async () => {
    ...
    try {
      ...
      setError(
        error instanceof Error ? error.message : 'GPS 권한 요청에 실패했습니다.'
      );
    } finally {
      setIsLoading(false);
    }
