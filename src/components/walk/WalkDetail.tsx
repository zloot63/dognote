    ...
    useEffect(() => {
        ...
        const fetchWalk = async () => {
            ...
            if (walkData) {
                ...
                console.error("🚨 산책 데이터를 불러오지 못했습니다.");
                router.push("/walks"); // 데이터 없을 시 목록 페이지로 이동
            }
        };
        fetchWalk();
    }, [walkId, router]);
    ...
    return (
        <div className="p-5 max-w-2xl mx-auto">
            ...
            <div className="bg-white shadow-md p-4 rounded-lg">
                <p><strong>강아지 ID:</strong> {walk.dogIds.join(", ")}</p>
                <p><strong>시작 시간:</strong> {new Date(walk.startTime).toLocaleString()}</p>
                <p><strong>종료 시간:</strong> {walk.endTime ? new Date(walk.endTime).toLocaleString() : "진행 중"}</p>
