...
const DatePicker = React.forwardRef<HTMLDivElement, DatePickerProps>(
  (
    ...
    const formatDate = React.useCallback(
      (date: Date | undefined) => {
        if (!date) return '';
        const formatStr = showTimePicker
          ? `${formatString} HH:mm`
          : formatString;
            ...
            >
              ...
              {clearable && date && !disabled && (
                <div
                  role="button"
                  tabIndex={0}
                  className="h-4 w-4 p-0 opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer flex items-center justify-center rounded-sm hover:bg-accent hover:text-accent-foreground"
                  onClick={handleClear}
                  onKeyDown={e => {
        ...
        <Popover open={open} onOpenChange={setOpen}>
          <PopoverTrigger asChild>
            <Button
              ...
              aria-haspopup="dialog"
              aria-label={
                dateRange ? formatDateRange(dateRange) : placeholders.full
              }
              aria-invalid={!!error}
              aria-describedby={error ? errorId : undefined}
              className={cn(
