...
_승인자: Tech Lead, Solution Architect_
...
DogNote는 **서버리스 우선(Serverless-First)** 아키텍처를 채택하여 빠른 개발과 자동 확장을
추구합니다. Firebase BaaS를 중심으로 한 모던 JAMstack 구조로 설계되었습니다.
...
export class Walk {
  ...
  static create(data: CreateWalkData): Walk {
    return new Walk(generateId(), data.dogIds, data.startedAt, 'active');
  }
...
_본 문서는 시스템 진화에 따라 지속적으로 업데이트됩니다._
