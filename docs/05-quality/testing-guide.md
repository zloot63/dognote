...
*DogNote 프로젝트의 포괄적인 테스팅 전략 및 구현 가이드*
...
describe('Accessibility', () => {
  it('should be navigable with keyboard only', () => {
    ...
    cy.get('[role="button"], button, [role="link"], a, input, select, textarea')
      .each(($el) => {
...
*좋은 테스트는 개발자의 자신감을 높이고 리팩토링을 두려워하지 않게 해줍니다.*
